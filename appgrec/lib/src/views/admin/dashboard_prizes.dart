...
class DashboardPrizesScreenState extends State<DashboardPrizesScreen> {
  ...
  bool isLoading = true;  // Nuevo estado para controlar la carga
  ...
  Future<void> _loadPrizes() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          ...
          if (data['data'] is List) {
            setState(() {
              prizes = List<String>.from(data['data']); // Solo la lista de premios sin 'data'
              filteredPrizes = List.from(prizes);
              isLoading = false;  // Al terminar la carga, cambiamos el estado
            });
          } else {
      ...
      if (mounted) {
        ...
        setState(() {
          isLoading = false;  // También desactivamos el cargador en caso de error
        });
      }
    }
  }
  ...
  void _handleSavePrize(String name, String description, File? image) {
    ...
    CustomSnackbar.showSuccess(
      context, 
      'Premio "$name" agregado con éxito!',
    );
  }
  ...
  void _handleDeletePrize(String prize) {
    ...
    CustomSnackbar.showSuccess(
      context, 
      'Premio "$prize" eliminado con éxito!',
    );
  }
            ...
            child: isLoading
                ? Center(
                    child: CircularProgressIndicator(
                      valueColor: AlwaysStoppedAnimation<Color>(Color(0xFFFF0000)),  // Rojo
                    ),
                  )
                : (prizes.isEmpty)
                    ? Center(
                    ...
                    : isEditing
                        ? ListView.builder(
                            itemCount: visiblePrizes.isEmpty ? 1 : visiblePrizes.length, // Si no hay premios, se agrega un item para el mensaje
                            itemBuilder: (context, index) {
                              if (visiblePrizes.isEmpty) {
                                return Center(
                                  child: Text(
                                    'Aún no hay premios creados',
                                    style: TextStyle(
                        ...
                        : ListView.builder(
                            itemCount: visiblePrizes.isEmpty ? 1 : visiblePrizes.length + 1, // Si no hay premios, se agrega un item para el mensaje
                            itemBuilder: (context, index) {
                              if (visiblePrizes.isEmpty) {
                                return Center(
                                  child: Text(
                              ...
                              } else if (index < visiblePrizes.length) {
                                bool isSelected = visiblePrizes[index] == selectedPrize;
                                return ListTile(
                                  title: Text(
                                    visiblePrizes[index],
